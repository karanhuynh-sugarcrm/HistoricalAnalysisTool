...
module.exports = {
	 ...
	 * @param {Object} Data to generate the home template data that feeds into the handlebars templates
	 * @param {Function} The success callback function
	 * @param {Function} The failure callback function
	 */
	generateDiff : function(urls, res, home, success, failure) { 
		...
		async.map(urls, fetch, function(error, results) {
			if(error) {
				...
				home.tests = new AllureDiff(results, home.singleTestSearch).statusDiff();
			}
			else {
				home.tests = new AllureDiff(results).statusDiff();
